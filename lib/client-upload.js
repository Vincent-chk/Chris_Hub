...
export async function uploadCroppedImage({
  ...
  // 导出后、上传前的大小预检：超限直接拒绝，不产生 OSS 对象
  if (typeof maxBytes === "number" && blob.size > maxBytes) {
    const limitMb = Math.round(maxBytes / (1024 * 1024));
    throw new Error(
      `导出后的图片为 ${(blob.size / (1024 * 1024)).toFixed(1)}MB，超过上限 ${limitMb}MB，请选择分辨率或细节更小的图片`,
    );
  }
  ...
  try {
    ...
    return await validateUpload({ accessKey, objectKey: token.objectKey, specId, checksum });
  } catch (err) {
