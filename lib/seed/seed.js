  ...
  db.transaction((tx) => {
    ...
    for (const product of PRODUCTS) {
      product.skus.forEach((sku, skuIndex) => {
        ...
        sku.images.forEach((image, imageIndex) => {
    ...
    tx.insert(banners)
      .values(
        BANNERS.map((banner, index) => ({
          id: banner.id,
          desktopImageCnKey: banner.cn.replace(/^\//, ""),
          desktopImageEnKey: banner.en.replace(/^\//, ""),
          mobileImageCnKey: banner.cnMobile.replace(/^\//, ""),
          mobileImageEnKey: banner.enMobile.replace(/^\//, ""),
          sortOrder: index,
          enabled: 1,
        })),
      )
