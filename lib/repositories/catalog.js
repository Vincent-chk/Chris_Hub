...
function buildCardSummaries(productRows, locale) {
  ...
  return productRows.map((product) => {
    ...
    const defaultImageKey = defaultSku?.cardImageObjectKey || defaultImage?.objectKey;
    ...
    return {
      ...
      tags: localizedTags,
      coverUrl: defaultImageKey ? assetUrl(defaultImageKey) : null,
...
export function getHomeData(locale) {
  ...
  function bannerUrls(purpose) {
    return bannerRows.filter((row) => row.purpose === purpose).map((row) => assetUrl(row.objectKey));
  }

  const desktopList = bannerUrls(l === "cn" ? "cn-desktop" : "en-desktop");
  const mobileList = bannerUrls(l === "cn" ? "cn-mobile" : "en-mobile");
  const bannerList = {
    desktop: desktopList,
    mobile: mobileList.length ? mobileList : desktopList,
  };
  ...
  return {
    banners: bannerList,
    popularProducts: buildCardSummaries(productRows, l),
    contact,
    logoUrl: assetUrl(settingsRow?.logoObjectKey ?? null),
  };
}

// 网站设置（阶段 C · C6）：Logo 与联系信息，供各页面/组件使用
export function getSiteSettings(locale) {
  const l = localeOf(locale);
  const row = db.select().from(siteSettings).where(eq(siteSettings.id, 1)).get();
  return {
    logoUrl: assetUrl(row?.logoObjectKey ?? null),
    contact: {
      description: localized({ cn: row?.contactTextCn ?? "", en: row?.contactTextEn }, l),
      wechatId: row?.wechatId ?? "",
      qrUrl: assetUrl(row?.wechatQrObjectKey ?? null),
    },
  };
}
...
export function getProductDetail({ productId, locale }) {
  const l = localeOf(locale);
  ...
  const skuRows = db
    .select()
    .from(skus)
    .where(and(eq(skus.productId, productId), eq(skus.enabled, 1)))
  ...
  for (const image of imageRows) {
    ...
    imagesBySku.get(image.skuId).push(assetUrl(image.objectKey));
  }
  for (const sku of skuRows) {
    if (!(imagesBySku.get(sku.id) ?? []).length) return null;
  }

  const tagRows = db
    .select({ tagId: productTags.tagId, nameCn: tags.nameCn, nameEn: tags.nameEn })
    .from(productTags)
    .innerJoin(tags, eq(productTags.tagId, tags.id))
    .where(and(eq(productTags.productId, productId), eq(tags.enabled, 1)))
    .all();

  const site = getSiteSettings(l);
  return {
    id: product.id,
    name: localized({ cn: product.nameCn, en: product.nameEn }, l),
    description: localized({ cn: product.descriptionCn, en: product.descriptionEn }, l),
    tags: tagRows.map((tag) => localized({ cn: tag.nameCn, en: tag.nameEn }, l)),
    skus: skuRows.map((sku) => ({
      id: sku.id,
      name: localized({ cn: sku.nameCn, en: sku.nameEn }, l),
      tab: localized({ cn: sku.tabLabelCn, en: sku.tabLabelEn }, l),
      price: sku.priceCents,
      cardImage: sku.cardImageObjectKey ? assetUrl(sku.cardImageObjectKey) : (imagesBySku.get(sku.id)?.[0] ?? null),
...
export function incrementProductView(productId) {
  ...
  try {
    ...
    console.error(`incrementProductView 失败: productId=${productId}`, error.message);
  }
}

export function listEnabledTags(locale) {
  const l = localeOf(locale);
  return db
    .select()
    .from(tags)
    ...
    .orderBy(tags.nameCn)
    .all()
    .map((tag) => ({ id: tag.id, name: localized({ cn: tag.nameCn, en: tag.nameEn }, l) }));
}
