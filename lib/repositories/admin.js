...
function bannerToShape(row) {
  return {
    id: row.id,
    desktopImageCn: { objectKey: row.desktopImageCnKey },
    desktopImageEn: { objectKey: row.desktopImageEnKey },
    mobileImageCn: row.mobileImageCnKey ? { objectKey: row.mobileImageCnKey } : null,
    mobileImageEn: row.mobileImageEnKey ? { objectKey: row.mobileImageEnKey } : null,
...
async function validateBannerImageRef(image, specId, validateImage, missingMessage) {
  if (!image || typeof image !== "object" || !image.objectKey) {
...
async function parseBannerInput(input, validateImage) {
  const data = input && typeof input === "object" ? input : {};
  const desktopCn = await validateBannerImageRef(
    data.desktopImageCn,
    "banner-desktop",
    validateImage,
    "缺少中文桌面图",
  );
  const desktopEn = await validateBannerImageRef(
    data.desktopImageEn,
    "banner-desktop",
    validateImage,
    "缺少英文桌面图",
  ...
  let mobileCn = null;
  let mobileEn = null;
  if (data.mobileImageCn && typeof data.mobileImageCn === "object") {
    mobileCn = await validateBannerImageRef(data.mobileImageCn, "banner-mobile", validateImage, "移动图无效");
  }
  if (data.mobileImageEn && typeof data.mobileImageEn === "object") {
    mobileEn = await validateBannerImageRef(data.mobileImageEn, "banner-mobile", validateImage, "移动图无效");
  }
  return {
    desktopImageCnKey: desktopCn.objectKey,
    desktopImageEnKey: desktopEn.objectKey,
    mobileImageCnKey: mobileCn?.objectKey ?? null,
    mobileImageEnKey: mobileEn?.objectKey ?? null,
...
export async function cleanupOrphanedBannerObjects(keys) {
  ...
  const rows = db.select().from(banners).all();
  const referenced = new Set();
  for (const row of rows) {
    for (const key of [
      row.desktopImageCnKey,
      row.desktopImageEnKey,
      row.mobileImageCnKey,
      row.mobileImageEnKey,
    ]) {
...
export async function createBanner(input, { validateImage = validateUploadedImage } = {}) {
  const count = Number(db.select({ count: sql`count(*)` }).from(banners).get().count);
  if (count >= MAX_BANNERS) {
    throw new ValidationError(`Banner 数量已达上限（${MAX_BANNERS} 张），请先删除旧 Banner 后再新增`);
  }
  const parsed = await parseBannerInput(input, validateImage);
  const maxRow = db
  ...
  const now = nowIso();
  db.insert(banners)
    .values({ id, ...parsed, sortOrder, createdAt: now, updatedAt: now })
    .run();
  return getBanner(id);
}
...
export async function updateBanner(
  ...
  const parsed = await parseBannerInput(input, validateImage);
  const oldKeys = [
    existing.desktopImageCnKey,
    existing.desktopImageEnKey,
    existing.mobileImageCnKey,
    existing.mobileImageEnKey,
  ];
  db.update(banners)
    ...
    .run();
  const newKeys = [
    parsed.desktopImageCnKey,
    parsed.desktopImageEnKey,
    parsed.mobileImageCnKey,
    parsed.mobileImageEnKey,
  ];
  const stale = oldKeys.filter((key) => key && !newKeys.includes(key));
  if (stale.length) await cleanupObjects(stale);
  return getBanner(id);
}
...
export function reorderBanners(ids) {
  ...
  const existing = db.select({ id: banners.id }).from(banners).where(inArray(banners.id, ids)).all();
  if (existing.length !== ids.length) {
    throw new ValidationError("包含不存在的 Banner");
  }
  db.transaction((tx) => {
...
export async function deleteBanner(id, { cleanupObjects = cleanupOrphanedBannerObjects } = {}) {
  const existing = db.select().from(banners).where(eq(banners.id, id)).get();
