...
// 某标签绑定的商品基本信息（含草稿与缩略图，按更新时间倒序）；标签不存在返回 null（阶段 C · C7）
export function listTagProducts(tagId) {
  ...
  // 缩略图：优先启用 SKU 的列表缩略图，回退其第一张详情大图；无则 null
  const productIds = rows.map((row) => row.id);
  const skuRows = productIds.length
    ? db
        .select()
        .from(skus)
        .where(and(inArray(skus.productId, productIds), eq(skus.enabled, 1)))
        .orderBy(skus.position)
        .all()
    : [];
  const enabledSkuIds = skuRows.map((sku) => sku.id);
  ...
  for (const image of imageRows) {
    if (!firstImageBySku.has(image.skuId)) firstImageBySku.set(image.skuId, image.objectKey);
  }
  const thumbByProduct = new Map();
  for (const sku of skuRows) {
    if (thumbByProduct.has(sku.productId)) continue;
    const key = sku.cardImageObjectKey || firstImageBySku.get(sku.id);
    if (key) thumbByProduct.set(sku.productId, key);
  }
  ...
  return rows.map((row) => ({
      ...
      status: row.status,
      updatedAt: row.updatedAt,
      thumbnailUrl: assetUrl(thumbByProduct.get(row.id) ?? null),
    }));
}
