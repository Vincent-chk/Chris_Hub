...
export default function ProductDetail({ locale, product }) {
  ...
  function shiftImage(delta) {
    setImageIndex((value) => (value + delta + sku.detailImages.length) % sku.detailImages.length);
  }
      ...
      <main className="detail-main section-pad">
        ...
        <div className="detail-layout">
          ...
          <section className="gallery" aria-label="Product images">
            ...
            <div className="thumbnail-row">{sku.detailImages.map((item, index) => <button className={`thumbnail ${index === imageIndex ? "is-active" : ""}`} type="button" key={item} onClick={() => setImageIndex(index)}><img src={item} alt="" /></button>)}</div>
          </section>
        </div>
      </main>
