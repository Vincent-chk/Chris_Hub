...
8. [x] 全局孤儿对象清理（补充任务：`scripts/cleanup-orphans.mjs` CLI 脚本 + 中台"开发者运维"页检测/清理；默认 dry-run，CLI 加 `--apply` 或中台人工确认后才删除；扫描 OSS 桶与数据库 4 类引用比对，仅删 `sku/ banner/ site/` 前缀未引用对象；使用时机/发起人/验收标准/用户交互见脚本头部注释）。
