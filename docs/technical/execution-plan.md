...
3. [x] Banner 管理（C4：上限 5 张、中英桌面图必填、移动图可选、排序/启停/删除、前台即时生效）；网站设置（C6：Logo/微信号/微信二维码/中英联系说明，保存后前台全站生效，未配置时回退默认 Logo 与占位图）；标签管理页待后续任务。
...
> C3 说明：独立标签管理页（列表/编辑/启停）留待 C4；本地开发期前台读取 OSS 私有桶图片走 `/oss/` 代理（`app/oss/[...key]/route.js`），生产使用 CDN `ASSET_BASE_URL`。
