...
服务端按 `productId` 获取一个 Product 聚合及其启用 SKU、图片。客户端默认选择排序第一的 SKU；切换 SKU 同步更新 SKU 名称、价格和图片组，但 URL 不变。图片第一张是主图，支持缩略图、左右切换和放大查看。联系购买弹窗只展示中台配置的微信号和二维码。
...
- 图片容器始终使用 `aspect-ratio`，主图、缩略图和卡片不会因文字或加载状态改变尺寸。
